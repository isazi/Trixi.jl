...
@inline function _exp_ijk_fusedloop_calc_volume_integral!(backend::Backend, du, u,
                                                mesh::P4estMesh{3},
                                                nonconservative_terms::False, equations,
                                                volume_integral::VolumeIntegralFluxDifferencing,
                                                dg::DGSEM, cache, default_wgs)
...
@kernel function _exp_ijk_fusedloop_flux_differencing_kernel!(du, u, equations,
                                                volume_flux, num_nodes, derivative_split,
                                                contravariant_vectors, alpha = true)
    # true * [some floating point value] == [exactly the same floating point value]
    # This can (hopefully) be optimized away due to constant propagation.
...
@inline function _exp_ijk_split_calc_volume_integral!(backend::Backend, du, u,
    ...
    nodes = eachnode(dg)
    num_nodes = length(nodes)
    kernel_x! = _exp_ijk_x_flux_differencing_kernel!(backend)
    kernel_y! = _exp_ijk_y_flux_differencing_kernel!(backend)
    kernel_z! = _exp_ijk_z_flux_differencing_kernel!(backend)
...
@kernel function _exp_ijk_x_flux_differencing_kernel!(du, u, equations,
                                                volume_flux, num_nodes, derivative_split,
                                                contravariant_vectors, alpha = true)
    # true * [some floating point value] == [exactly the same floating point value]
    # This can (hopefully) be optimized away due to constant propagation.
...
@kernel function _exp_ijk_y_flux_differencing_kernel!(du, u, equations,
                                                volume_flux, num_nodes, derivative_split,
                                                contravariant_vectors, alpha = true)
    # true * [some floating point value] == [exactly the same floating point value]
    # This can (hopefully) be optimized away due to constant propagation.
...
@kernel function _exp_ijk_z_flux_differencing_kernel!(du, u, equations,
                                                volume_flux, num_nodes, derivative_split,
                                                contravariant_vectors, alpha = true)
    # true * [some floating point value] == [exactly the same floating point value]
    # This can (hopefully) be optimized away due to constant propagation.
...
@inline function _exp_ijk_nosym_split_calc_volume_integral!(backend::Backend, du, u,
                                                mesh::P4estMesh{3},
                                                nonconservative_terms::False, equations,
                                                volume_integral::VolumeIntegralFluxDifferencing,
                                                dg::DGSEM, cache, default_wgs)
    ...
    nodes = eachnode(dg)
    num_nodes = length(nodes)
    kernel_x! = _exp_ijk_nosym_x_flux_differencing_kernel!(backend)
    kernel_y! = _exp_ijk_nosym_y_flux_differencing_kernel!(backend)
    kernel_z! = _exp_ijk_nosym_z_flux_differencing_kernel!(backend)
...
@kernel function _exp_ijk_nosym_x_flux_differencing_kernel!(du, u, equations,
                                                volume_flux, num_nodes, derivative_split,
                                                contravariant_vectors, alpha = true)
    # true * [some floating point value] == [exactly the same floating point value]
    # This can (hopefully) be optimized away due to constant propagation.
...
@kernel function _exp_ijk_nosym_y_flux_differencing_kernel!(du, u, equations,
                                                volume_flux, num_nodes, derivative_split,
                                                contravariant_vectors, alpha = true)
    # true * [some floating point value] == [exactly the same floating point value]
    # This can (hopefully) be optimized away due to constant propagation.
...
@kernel function _exp_ijk_nosym_z_flux_differencing_kernel!(du, u, equations,
                                                volume_flux, num_nodes, derivative_split,
                                                contravariant_vectors, alpha = true)
    # true * [some floating point value] == [exactly the same floating point value]
    # This can (hopefully) be optimized away due to constant propagation.
...
@inline function _exp_ijk_nosym_fusedloop_inter_calc_volume_integral!(backend::Backend, du, u,
                                                mesh::P4estMesh{3},
                                                nonconservative_terms::False, equations,
                                                volume_integral::VolumeIntegralFluxDifferencing,
                                                dg::DGSEM, cache, default_wgs)
    ...
    nodes = eachnode(dg)
    num_nodes = length(nodes)
    kernel! = _exp_ijk_nosym_fusedloop_inter_flux_differencing_kernel!(backend)
...
@kernel function _exp_ijk_nosym_fusedloop_inter_flux_differencing_kernel!(du, u, equations,
                                                volume_flux, num_nodes, derivative_split,
                                                contravariant_vectors, alpha = true)
    # true * [some floating point value] == [exactly the same floating point value]
    # This can (hopefully) be optimized away due to constant propagation.
    ...
    @unroll for other in 1:num_nodes
        ...
        Ja_node_other = get_contravariant_vector(1, contravariant_vectors,
                                                other, j, k, element)
        Ja_avg = 0.5 * (Ja1_node + Ja_node_other)
        fluxtilde = volume_flux(u_node, u_node_other, Ja_avg, equations)
        multiply_add_to_first_axis!(du, alpha * derivative_split[i, other], fluxtilde,
                                    i, j, k, element)
        u_node_other = get_svector(u, NVARS, i, other, k, element)
        Ja_node_other = get_contravariant_vector(2, contravariant_vectors,
                                                i, other, k, element)
        Ja_avg = 0.5 * (Ja2_node + Ja_node_other)
        fluxtilde = volume_flux(u_node, u_node_other, Ja_avg, equations)
        multiply_add_to_first_axis!(du, alpha * derivative_split[j, other], fluxtilde,
                                    i, j, k, element)
        u_node_other = get_svector(u, NVARS, i, j, other, element)
        Ja_node_other = get_contravariant_vector(3, contravariant_vectors,
                                                i, j, other, element)
        Ja_avg = 0.5 * (Ja3_node + Ja_node_other)
        fluxtilde = volume_flux(u_node, u_node_other, Ja_avg, equations)
        multiply_add_to_first_axis!(du, alpha * derivative_split[k, other], fluxtilde,
                                    i, j, k, element)
    end
end
